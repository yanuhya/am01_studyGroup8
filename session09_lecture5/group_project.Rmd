...
Let's do a general analysis to see how respondents are divided among gender, race and industry.

Can we suggest any racial or gender discrimination going on in relation to the salaries people have entered? If so, is this discrimination country or industry-specific? Also, do salaries vary across industries? Is there variability in these industries?
...
Respondents of the survey were predominantly white. Let us see if there is any noticeable difference in the pays of men and women, and cross-check that with null hypothesis
...
ggplot(ask_a_manager_2021 %>% filter(gender == "Man" | gender == "Woman"), 
  ...
  scale_x_log10() +
  xlim(0,400000) +
  labs(title = "Pay difference by gender", x = "Salary in USD", y = "Gender") + 
  geom_boxplot() 
```
...
ask_a_manager_2021 %>% 
  ...
  xlim(0,400000) +
  labs(title = "Pay difference by gender in different countries ", x = "Salary in USD", y = "Gender") + 
...
p1 <- ggplot(a, aes(x=reorder(country_code,median),y=median,size=median))+ 
  geom_point(aes(size=median,color=-1*log10(value)))+
  scale_size(rang=c(5,15))+
  scale_colour_gradient(low="mediumblue",high="pink")+
  labs(color=expression(-log[10](Pvalue)),size="Median",  
       x="Country",y="Median Salary",title="Median Salary in different countries")
 p1
```
...
ggplot(ask_a_manager_2021_state_salary) + 
  ...
  labs(title = "Mean salary for each states in the US") +
  NULL
```
